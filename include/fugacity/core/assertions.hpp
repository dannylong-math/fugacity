...
/**
 * @file assertions.hpp
 * @brief Debug-only precondition macro that throws instead of aborting.
 */
 ...
 * is compiled out entirely in release builds (@c NDEBUG defined), so it costs
 * nothing on the hot numerical paths. Unlike @c assert, a failing check in a
 * debug build @b throws a @c std::logic_error rather than calling @c abort.
 *
 * Throwing (rather than aborting) is what makes the failure path testable: a
 * test can assert the throw with boost::ut's @c throws matcher, and — because
