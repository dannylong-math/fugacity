//
// Tests for fugacity::EoS construction preconditions.
//
...
int main()
{
    suite<"eos_pair"> s = [] {
        ...
        // the pair by throwing std::logic_error (FUGACITY_ASSERT, debug only).
        "mismatched component counts throw std::logic_error"_test = [] {
            ...
            expect(throws<std::logic_error>([&] {
                const fugacity::EoS<IdealGasTestModel<2>, VirialResidualTestModel<3>> bad{ideal, residual};
                (void)bad;
            }));
        };
#endif
